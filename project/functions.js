var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	core.waitHeroToStop(function () {
		...
		core.drawText([
			"\t[" + (reason || "恭喜通关") + (core.getFlag('GEL',0)?" (GEL)":"") + "]你的分数是${status:hp}。"
		], function () {
	...
	*/
	...
	if (core.status.floorId=="time") {
		if(!core.T)core.T=timestamp;else core.status.hero.mdef-=timestamp-core.T,core.T=timestamp;
		if(core.status.hero.mdef<=0)core.status.hero.mdef=0;
		core.updateStatusBar();
		if(core.status.hero.mdef<=0)core.insertAction([{"type": "playSound", "name": "gameover.mp3"},{"type":"if","condition":"flag:___==30000","true":[{"type":"win","reason":"30 sec"}]},{"type":"if","condition":"flag:___==60000","true":[{"type":"win","reason":"60 sec"}]}]);
	}
}
    },
    "ui": {
	...
	core.fillText('ui', "消灭星星", text_start, top+35, "#FFD700", "bold 22px "+globalFont);
	//core.fillText('ui', "版本： "+main.__VERSION__, text_start, top + 80, "#FFFFFF", "bold 17px "+globalFont);
	core.fillText('ui', "作者： bdf", text_start, top + 112);
